...
export const ClassTable: FC<ClassTableType> = memo((props: ClassTableType) => {
  ...
  const isFullSpellCaster =
    isBard || isCleric || isWizard || isDruid || isSorcerer;
  const isHalfSpellCaster = isRanger || isArtificer || isPaladin;
  const startCantrips = isSorcerer ? 4 : isCleric || isWizard ? 3 : 2;
  ...
  return (
    <div className={cls.classTable}>
      <table>
        <thead>
          <tr>
            <th rowSpan={2} className={cls.lvlCol}>
              Рів.
            </th>
            <th rowSpan={2} className={cls.bmCol}>
              БМ
            </th>
            <th rowSpan={2} className={cls.featCol}>
              Вміння
            </th>
            {isArtificer && <th rowSpan={2}>Відомі Інфузії</th>}
            {isArtificer && <th rowSpan={2}>Інфузії предметів</th>}
            {isKnowCantrips && <th rowSpan={2}>Відомі замовляння</th>}
            {isKnowSpells && <th rowSpan={2}>Відомі Чари</th>}
            {(isFullSpellCaster || isHalfSpellCaster) && (
              <th align="center" colSpan={isHalfSpellCaster ? 5 : 9}>
                Кількість доступних чарунків на рівень
              </th>
            )}
            {isBarbarian && <th>Лють</th>}
            {isBarbarian && <th>Шкода Люті</th>}
          </tr>
          {isFullSpellCaster && (
            <tr>
              <th>1</th>
        ...
        <tbody>
          {featuresArr.map((level, i) => {
            ...
            return (
              <tr key={i}>
                <td align="center" className={cls.lvlCol}>
                  {i + 1}
                </td>
                <td align="center" className={cls.bmCol}>
                  +{bm}
                </td>
                <td className={cls.featCol}>
                  {level.map((feat, index) => (
                    <span key={feat.name} className={cls.skill}>
                      {feat.name}
